...
class Register extends Component
{
    ...
    public function registerNewUser()
    {
        ...
        $email = $user->email;
        $data = [];
        $data = array('username' => $username, 'verified_code' => $user->verified_code);
        // Mail::send('send-verifaction-email', $data , function($message) use ($username ,$email){
        //     $message->to($email,$username)->subject('ارسال کد فعالسازی');
        //     $message->from('amirmahdiakhi@gmail.com','codeVerify');
        // });
        // Mail::to($email,$username)->send(new sendVerifactionCodeMail($user));
