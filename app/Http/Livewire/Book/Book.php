...
class Book extends Component
{
    ...
    public function sendLike() {
        if($this->isLiked == true){
            $bookId = Like::where(['user_id' => $this->user->id , 'book_id' => $this->book->id])->orderBy('created_at','desc')->first()->book_id;
        ...
        elseif ($this->isLiked == false) {
            ...
            $bookId = Like::where('user_id' , $this->user->id)->orderBy('created_at','desc')->first()->book_id;
            $book = new ModelsBook;
            $book = $book->find($bookId);
            $likeNum = $book->likes;
            $likes = $likeNum + 1;
